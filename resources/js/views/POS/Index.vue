<template>
    <div class="min-h-screen flex flex-col bg-gray-50 pos-container">
        ...
        <div
            class="md:hidden bg-white border-b border-gray-200 mobile-nav-sticky"
        >
            <div class="flex">
                <button
        ...
        <div class="flex-1 flex p-3 md:p-6 gap-3 md:gap-6">
            ...
            <div
                :class="
                    mobileActiveTab === 'products' || !isMobile
                        ? 'flex'
                        : 'hidden'
                "
            ...
            >
                ...
                <div class="p-3 md:p-4 border-b border-gray-200">
                    ...
                    >
                        ...
                        <div
                            v-if="selectedClient"
                            class="flex items-center text-sm bg-blue-50 px-3 py-2 rounded-lg"
                        >
                        ...
                        <div
                            v-else
                            class="text-sm text-blue-600 font-medium bg-blue-50 px-3 py-2 rounded-lg"
                        >
                            Client de passage
                        </div>
                    </div>
                ...
                <div class="p-3 md:p-4 border-b border-gray-200">
                    <h2
                        class="text-base font-medium text-gray-900 mb-3 md:mb-4"
                    >
                    ...
                    <!-- Boutons d'action -->
                    <div
                        class="grid grid-cols-2 md:flex md:space-x-3 gap-2 md:gap-0 mb-3 md:mb-4"
                    >
                        <button
            ...
            <div
                :class="
                    mobileActiveTab === 'cart' || !isMobile ? 'flex' : 'hidden'
                "
            ...
            >
                ...
                <div class="flex-1 flex flex-col min-h-0">
                    <div class="flex-1 pos-scrollable p-3 md:p-4">
                        <div
                            v-if="cartItems.length === 0"
                            class="flex flex-col items-center justify-center text-gray-500 py-8"
                        >
                            <svg
                        ...
                        <div v-else class="space-y-2">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span
                                        class="text-xs font-medium text-gray-900 min-w-[1rem] text-center"
                                        >{{ item.quantity }}</span
                                    >
                                    <button
                                        @click="
                ...
                <div
                    class="border-t border-gray-200 p-3 space-y-3 flex-shrink-0"
                >
                    <!-- Totaux -->
                    ...
                    <div class="space-y-2.5">
                            ...
                            >
                                <div class="space-y-1 text-sm">
                                    <div class="flex justify-between">
                                        ...
                                        >
                                        <span
                                            class="font-medium text-orange-900"
                                            >{{
                                                formatPrice(totalAmount)
                                            }}</span
                                        >
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <span
                                            class="font-medium text-orange-900"
                                            >{{
                                                formatPrice(advanceAmount || 0)
                                            }}</span
                                        >
                                    </div>
                                    <div
                                        class="border-t border-orange-200 pt-1"
                                    >
                                        <div class="flex justify-between">
                                            <span
                                            ...
                                            >
                                            <span
                                                class="font-bold text-orange-900"
                                                >{{
                                                    formatPrice(
                                                        Math.max(
                                                            0,
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            ...
                            >
                                ...
                                <span v-else-if="paymentMode === 'complete'"
                                    >Valider Complet</span
                                >
                                <span v-else-if="paymentMode === 'partial'"
                                    >Valider Partiel</span
                                >
            ...
            >
                ...
                >
                    ...
                    <span
                        class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold cart-badge"
                    >
                        {{ cartItems.length }}
                    </span>
                </button>
            ...
            >
                ...
                >
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div
                            v-else-if="clientSearch && clientSearch.length > 2"
...
const filteredProducts = computed(() => {
    ...
    if (currentFilter.value) {
        filtered = filtered.filter(
            (product) => product.type === currentFilter.value
        );
    }
...
const finalizeSalePartial = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            lastSaleId.value =
                response.data.sale?.id || response.data.data?.id || Date.now();
...
const getCartItemColorClass = (item) => {
    ...
    if (description.includes("verre") || description.includes("lens"))
        return "bg-purple-500";
    if (description.includes("monture") || description.includes("frame"))
        return "bg-green-500";
    if (description.includes("chiffon") || description.includes("accessoire"))
        return "bg-red-500";
    if (description.includes("lentille") || description.includes("contact"))
        return "bg-blue-500";
...
const formatLensDetails = (details) => {
    const parts = [];
    if (details.sph !== undefined)
        parts.push(`SPH: ${details.sph >= 0 ? "+" : ""}${details.sph}`);
    if (details.cyl !== undefined)
        parts.push(`CYL: ${details.cyl >= 0 ? "+" : ""}${details.cyl}`);
...
const handlePayment = async () => {
    ...
    if (paymentMode.value === "partial") {
        if (!advanceAmount.value || advanceAmount.value <= 0) {
            alert(
                "Veuillez saisir un montant d'avance valide pour le paiement partiel."
            );
            return;
        }
        if (advanceAmount.value > totalAmount.value) {
            alert(
                "Le montant d'avance ne peut pas être supérieur au total à payer."
            );
            return;
        }
    }
...
const handlePrintConfirmation = (confirmation) => {
    ...
    if (confirmation.printType !== "none") {
        try {
            ...
            console.log(
                "Index - handlePrintConfirmation - Données reçues:",
                confirmation
            );
            console.log(
                "Index - handlePrintConfirmation - saleData du confirmation:",
                confirmation.saleData
            );
            console.log(
                "Index - handlePrintConfirmation - saleData stocké:",
                confirmationData.value.saleData
            );

            // Utiliser les données complètes (priorité à celles du confirmation)
            const saleData =
                confirmation.saleData || confirmationData.value.saleData;

            if (saleData) {
                // Vérifier que les articles sont présents
                console.log(
                    "Index - handlePrintConfirmation - Articles:",
                    saleData.items
                );
                ...
                console.error(
                    "Données de vente non disponibles pour impression"
                );
                alert(
                    "Erreur: Données de vente non disponibles pour impression"
                );
            }
        } catch (error) {
            console.error(
...
const openPrintModal = (type) => {
    try {
        ...
        console.error(
            "Erreur lors de l'ouverture du modal d'impression:",
            error
        );
        alert(
...
const preparePrintDataFromConfirmation = (confirmationData, printOptions) => {
    return {
        ...
        date: new Date(),
        vendeur: "Système", // À remplacer par l'utilisateur connecté
        client: confirmationData.clientName
            ? { name: confirmationData.clientName }
            : null,
        items: [], // Les items ont été réinitialisés, on utilise les données sauvegardées
        paymentMethod:
            confirmationData.operationType === "sale" ? "complete" : null,
