  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b border-gray-200 p-4 md:p-6">
      <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
        <div>
          <h1 class="text-xl md:text-2xl font-bold text-gray-900">Gestion du Stock</h1>
          <p class="text-gray-600 mt-1 text-sm md:text-base">
            Gérez votre stock directement - {{ products.length }} produit(s)
          </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
          ...
          >
            ...
            <span class="hidden sm:inline">Ajouter un </span>accessoire
          </button>
        </div>
      </div>
    </div>
    ...
    <!-- Message de succès -->
    <div v-if="successMessage" class="mx-4 md:mx-6 mt-4 p-4 bg-green-50 border border-green-200 rounded-md">
      <div class="flex">
    ...
    <!-- Filtres -->
    <div class="mx-4 md:mx-6 mt-6 bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <!-- Contenu des filtres -->
      <div :class="showFilters || !isMobile ? 'block' : 'hidden'" class="p-4">
        <div class="flex flex-col md:flex-row md:flex-wrap gap-4">
          <!-- Toggle vue liste/grille sur mobile -->
          <div v-if="isMobile" class="flex items-center space-x-2 md:hidden">
    ...
    <div class="mx-4 md:mx-6 mt-6 pb-6">
      ...
      <div v-else-if="filteredProducts.length === 0" class="text-center py-16">
        ...
        <p class="mt-2 text-gray-500">
          {{ searchQuery || selectedCategoryId || stockFilter ? 'Aucun produit ne correspond à vos critères de recherche.' : 'Commencez par ajouter des produits à votre stock.' }}
        </p>
      </div>
      ...
      <div
        v-else
        :class="viewMode === 'list' && isMobile ? 'space-y-3' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6'"
      >
        <div
          ...
          <div class="p-3 md:p-4">
            ...
            <div class="flex justify-between items-start mb-3">
              <div class="flex-1 min-w-0">
                <h3 class="text-sm md:text-base font-semibold text-gray-900 truncate">{{ product.name }}</h3>
                <p v-if="product.brand" class="text-xs text-gray-500 mt-1">{{ product.brand }}</p>
              </div>
            ...
            <div class="space-y-2">
              ...
              <div class="flex justify-between items-center">
                ...
                <span 
                  ...
                  :class="getStockColorClass(product.quantity)"
                >
                  {{ product.quantity || 0 }}
                </span>
              </div>

              <div v-if="product.productCategory" class="flex justify-between items-center">
                <span class="text-xs text-gray-500">Catégorie:</span>
                <span class="text-xs text-gray-700">{{ product.productCategory.name }}</span>
              </div>
              ...
              <div class="mt-3">
                <span 
                  ...
                  :class="getStockBadgeClass(product.quantity)"
                >
    ...
    <div v-if="isMobile" class="fixed bottom-6 right-6 z-50">
      <div class="relative">
        ...
        >
          ...
          <button
            @click="openAddAccessoryModal(); showFloatingMenu = false"
...
const filteredProducts = computed(() => {
  ...
  if (selectedCategoryId.value) {
    filtered = filtered.filter(product => product.product_category_id === selectedCategoryId.value)
  }
