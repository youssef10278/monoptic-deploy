...
watch(() => props.product, (newProduct) => {
  if (newProduct) {
    form.value = {
      ...
      product_category_id: newProduct.product_category_id || props.selectedCategoryId || ''
    }
  } else {
    resetForm()
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      emit('success', {
        message: isEditing.value ? 'Produit modifié avec succès' : 'Produit créé avec succès',
    ...
    if (error.response?.data?.errors) {
      ...
      errorMessage.value = `Erreur lors de la ${isEditing.value ? 'modification' : 'création'} du produit`
    }
  } finally {
